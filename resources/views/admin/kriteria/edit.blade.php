...
@section('container')
    <div class="max-w-3xl">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-semibold text-slate-900">Edit Kriteria</h1>
                <p class="mt-1 text-sm text-slate-600">Perbarui data kriteria.</p>
            </div>
        ...
        <form action="{{ route('admin.kriteria.update', $kriteria) }}" method="POST"
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input name="kode_kriteria" value="{{ old('kode_kriteria', $kriteria->kode_kriteria) }}"
                ...
                <div>
                    ...
                    <input name="bobot" value="{{ old('bobot', $kriteria->bobot) }}" inputmode="decimal"
                        class="mt-1 h-10 w-full rounded-lg border border-slate-200 px-3 text-sm focus:border-slate-400 focus:outline-none" />
                </div>
                ...
                <div>
                    ...
                    <select name="jenis_atribut"
                        ...
                        <option value="benefit" @selected(old('jenis_atribut', $kriteria->jenis_atribut) === 'benefit')>benefit</option>
                        <option value="cost" @selected(old('jenis_atribut', $kriteria->jenis_atribut) === 'cost')>cost</option>
                    </select>
                </div>

                <div class="flex items-end">
                    <label class="inline-flex items-center gap-2 text-sm text-slate-700">
                        <input type="checkbox" name="status" value="1" @checked(old('status', $kriteria->status))
