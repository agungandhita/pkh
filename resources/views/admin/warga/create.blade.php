...
@section('container')
    <div class="max-w-3xl">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-semibold text-slate-900">Tambah Warga</h1>
                <p class="mt-1 text-sm text-slate-600">Input data alternatif calon penerima PKH.</p>
            </div>
        ...
        <form action="{{ route('admin.warga.store') }}" method="POST"
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="alamat" rows="3"
                        class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-slate-400 focus:outline-none">{{ old('alamat') }}</textarea>
                </div>
            ...
            <div class="mt-4">
                <label class="inline-flex items-center gap-2 text-sm text-slate-700">
                    <input type="checkbox" name="status_dtks" value="1" @checked(old('status_dtks'))
