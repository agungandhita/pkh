...
@section('container')
    <div class="space-y-4">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-xl font-semibold text-slate-900">Kriteria</h1>
                <p class="mt-1 text-sm text-slate-600">Kelola kriteria (benefit/cost), bobot, dan status aktif.</p>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-600">Kode</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-600">Nama</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-600">Atribut</th>
                            <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-600">Bobot</th>
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        @forelse ($kriteria as $row)
                            <tr class="hover:bg-slate-50">
                                <td class="whitespace-nowrap px-4 py-3 text-sm font-medium text-slate-900">{{ $row->kode_kriteria }}</td>
                                <td class="px-4 py-3 text-sm text-slate-900">{{ $row->nama_kriteria }}</td>
                                <td class="whitespace-nowrap px-4 py-3 text-sm text-slate-700">
                                    <span class="inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">
                                        {{ $row->jenis_atribut }}
                                    </span>
                                </td>
                                <td class="whitespace-nowrap px-4 py-3 text-sm text-slate-700">{{ number_format((float) $row->bobot, 4) }}</td>
                                <td class="whitespace-nowrap px-4 py-3 text-sm">
                                    @if ($row->status)
                                        <span class="inline-flex rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700">Aktif</span>
                                    @else
                                        <span class="inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">Nonaktif</span>
                                    @endif
                                </td>
                                <td class="whitespace-nowrap px-4 py-3">
                                    <div class="flex justify-end gap-2">
                                        <a href="{{ route('admin.kriteria.sub-kriteria.index', $row) }}"
                                        ...
                                        <a href="{{ route('admin.kriteria.edit', $row) }}"
                                            class="inline-flex h-9 items-center justify-center rounded-lg border border-slate-200 bg-white px-3 text-sm font-medium text-slate-700 hover:bg-slate-50">
                                            Edit
                                        </a>
                                        <form action="{{ route('admin.kriteria.destroy', $row) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-10 text-center text-sm text-slate-600">Belum ada data.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
